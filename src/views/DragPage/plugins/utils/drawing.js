// 获取point计算后样式
import {deepClone, getComponentProps, getElementConfig} from './util'
import {pContainer as plContainer} from './ele-config'
import {components} from '../../plugins/library/index'
import {moduleContainer} from './page-config'
...
export const getNodeElement = (nodeData, type = '') => {
  // 如果不存在该组件就直接返回
  if (!nodeData || !components.includes(nodeData.elName)) {
    Message.error({message: '没有该模块!', type: 'warning', duration: 2000})
    return null
  }
  let nodeElement
  // 获取需要添加的节点元素的数据结构
  nodeElement = getElementConfig(nodeData)

  // 注意: 如果我进来的不是容器，那么就需要包装一层容器，在返回节点
  // type如果存在，代表我是往容器里面加节点不需要被容器包裹，就不需要执行if语句了
  if (nodeElement.elName !== 'p-container' && type !== 'inner') {
    // 获取pl-container容器组件
    let containerNodeData = getElementConfig(plContainer)
    ...
    // 把需要添加的元素放入到容器节点中
    containerNodeData.elChild.push(nodeElement)
    // 导出容器
